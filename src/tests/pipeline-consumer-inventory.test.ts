...
test("the checked inventory exactly matches every workspace pipeline consumer", async () => {
  ...
  for (const manifestPath of manifests) {
    ...
    if (!/@(?:syrokomskyi|warpgogol)\/pipeline-(?:core|node)/.test(source)) continue;
    actual.push(
      path.relative(repositoryRoot, path.dirname(manifestPath)).split(path.sep).join("/"),
    );
  }
  ...
  for (const consumer of consumers) {
    ...
    const visit = async (directory: string): Promise<void> => {
      for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
        if (["node_modules", "dist", ".output"].includes(entry.name) || entry.name.startsWith("."))
          continue;
        ...
        if (entry.isDirectory()) await visit(absolute);
          ...
          legacy.test(await fs.readFile(absolute, "utf8"))
        )
          violations.push(path.relative(repositoryRoot, absolute));
      }
    };
    await visit(root);
  }
