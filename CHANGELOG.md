...
### Added
- Display concrete model IDs in AI Model Usage Guide tables.
- Generate a comprehensive 'all.md' pipeline guide containing all phases and steps.
- Require mandatory AI model usage disclosure in pipeline guides, following RFC-0007.

### Changed
- Upgrade and align dependencies across the workspace.

### Fixed
- Format the 'explainStepOverride' union type and provide a default implementation for 'getActiveArtifactIds'.
- Wire 'getActiveArtifactIds' into engine validation and reuse artifact presence checks.

### Removed
- Remove code-compass from all package.json devDependencies and scripts.
- Flatten project structure by moving all apps/source/* directories and files to the project root.

### Documentation
- Add and revise documentation for AI model usage disclosure (RFC-0007).
